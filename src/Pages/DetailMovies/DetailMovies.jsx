...
import {setDetailsMoviesAction} from "../../redux/actions/moviesActions";
import moment from "moment";
import ReactPlayer from "react-player";

function DetailMovies({maPhim}) {
    let {detailsMovies} = useSelector(state => state.moviesReducer)
    ...
    return (
        <div className='w-full bg-gradient-to-b from-sky-900 to-indigo-800 py-20 px-12'>
                    ...
                    <div className=''>
                        {
                            detailsMovies.dangChieu ? (
                                <div className='flex flex-col items-center'>
                                    <p className='py-6 text-lg text-white italic'>Ngày công chiếu: {moment(detailsMovies.ngayKhoiChieu).format('DD-MM-YYYY')}</p>
                                    <button className='bg-green-600 hover:bg-green-700 rounded-lg px-3 py-2 text-lg text-gray-50 transition-all duration-300'>Mua Vé</button>
                                </div>
                            ) : (
                                <div className='flex flex-col items-center'>
                                    <p className='py-6 text-lg text-white italic'>Ngày công chiếu: {moment(detailsMovies.ngayKhoiChieu).format('DD-MM-YYYY')}</p>
